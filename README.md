...
Run the Application
Make sure the data folder exists in your root directory before running the executable so the system can save the text files.
...
./clinic_system
📋 Interactive Menu
When you run the program, you will be greeted with the following CLI menu:
...
Search a doctor through their ID

Exit (Any other key)

🔮 Future Improvements
Integrate Appointment Menu: Link the existing backend logic in appointment.c to the main CLI menu so users can book and manage schedules interactively.
